---
title: Webhooks
description: Configure per-agent webhook delivery with optional HMAC-SHA256 signing for event notifications.
---
...
lets receivers verify the POST came from your gateway and
...
When present, every delivery includes:
...
is dropped silently. There is no retry queue.

If you need durable delivery, subscribe to the SSE stream
(`/anthropic/v1/sessions/:id/events/stream`) which is replay-safe via the
