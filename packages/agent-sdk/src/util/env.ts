...
export function upsertEnvLine(envPath: string, key: string, value: string): void {
  ...
  let updated: string;
  if (re.test(content)) {
    // Replace the FIRST occurrence; remove any subsequent duplicates
    let replaced = false;
    const lines = content.split("\n").filter((l) => {
      const isMatch = re.test(l);
      if (isMatch && !replaced) {
        replaced = true;
        return true; // keep this one (will be replaced below)
      }
      if (isMatch && replaced) return false; // remove duplicate
      return true;
    });
    updated = lines
      .map((l) => (!replaced ? l : (re.test(l) ? ((replaced = false), line) : l)))
      .join("\n");
    // Simpler approach: just replace all matches with the new value for the first, remove the rest
    updated = content.replace(re, line);
    // Remove any remaining duplicates
    const remaining = updated.split("\n");
    let seen = false;
    const deduped = remaining.filter((l) => {
      if (re.test(l)) {
