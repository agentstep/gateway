/**
 * Agent service — the business logic behind /v1/agents, decoupled from
 * HTTP. Handlers (and, later, the client's local transport) call these
 * functions with an AuthContext; parsing of *bodies* happens here so the
 * validation contract is identical for every caller, while URL/query
 * parsing and proxy forwarding stay in the handler codec.
 *
 * Proxy note: agents with engine "anthropic" are proxy-only resources.
 * `createAgentService` returns `{proxy: true}` to tell the codec to
 * forward the original request upstream — the service never talks HTTP.
 */
import { z } from "zod";
...
import { validateAnthropicProxy } from "../proxy/forward";
...
import type { Agent, AgentSkill, AuthContext } from "../types";
...
export function loadAgentForCaller(auth: AuthContext, id: string, version?: number): Agent {
...
const ModelConfigSchema = z.object({
  ...
  // Anthropic-compatible endpoint override — claude engine only (enforced
...
export type CreateAgentOutcome =
  /** Engine "anthropic" — the codec must forward the original request upstream. */
  | { proxy: true; tenantId: string | null }
  | { proxy: false; agent: Agent };

export async function createAgentService(auth: AuthContext, body: unknown): Promise<CreateAgentOutcome> {
  ...
  // Validate model is supported by this engine. A base-url override (claude
  // engine only) opens the model list to whatever the custom endpoint serves.
  const baseUrlOverride = !!parsed.data.model_config?.anthropic_base_url;
  if (baseUrlOverride && backendName !== "claude") {
    throw badRequest("model_config.anthropic_base_url is only supported on the claude engine");
  }
  ...
  return { proxy: false, agent };
}

export function listAgentsService(
  auth: AuthContext,
...
export function getAgentService(auth: AuthContext, id: string, version?: number): Agent {
  return loadAgentForCaller(auth, id, version);
}

export async function updateAgentService(auth: AuthContext, id: string, body: unknown): Promise<Agent> {
...
export function deleteAgentService(auth: AuthContext, id: string): { id: string; type: string } {
...
export function archiveAgentService(auth: AuthContext, id: string): Agent {
...
export function listAgentVersionsService(
  auth: AuthContext,
  id: string,
  opts: { limit: number; cursor?: number },
): ReturnType<typeof listAgentVersions> {
  loadAgentForCaller(auth, id); // tenant guard
  return listAgentVersions(id, { limit: opts.limit, cursor: opts.cursor });
}
